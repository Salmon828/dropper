...
namespace Unity.Multiplayer.PlayMode.Scenarios.Editor.GraphsFoundation
{
    ...
    {
        [SerializeField] internal ExecutionGraph m_ExecutionGraph;
        ...
        internal static Instance Create(InstanceDescription description = null, PlayModeController playModeController = null)
        {
        ...
        private async Task<bool> TryRunPreStartChecksAsync(CancellationToken cancellationToken)
        {
            ...
            {
                // instanceSimpleData is a simplified version of InstanceData,
                // containing only the type, build target, multiplayer role, and running mode
                var instanceSimpleData = GetInstanceSimpleData(this);
        ...
        internal static ErrorData GetValidationErrorData(Scenario.ValidationResult result)
        {
            ...
            {
                ExceptionType = typeof(ScenarioValidationException).ToString(),
                Message = result.Message,
            };
        }

        internal static InstanceData[] GetInstanceSimpleData(List<Instance> instances)
        {
            // To create a list of simplified InstanceData for all instances from scenario
            // containing only the Instance type, build target, multiplayer role, and running mode
            return instances.Select(GetInstanceSimpleData).SelectMany(data => data).ToArray();
        }

        internal static InstanceData[] GetInstanceSimpleData(Instance instance)
        {
            // To create a simplified list of InstanceData for free running instance
            // containing only the instance type, build target, multiplayer role, and running mode
            return new[]
            {
                new InstanceData
                {
                    Type = instance.m_InstanceDescriptionType,
                    BuildTarget = instance.m_BuildTarget,
                    RunningMode = instance.IsFreeRunMode()? RunModeState.ManualControl.ToString() : RunModeState.ScenarioControl.ToString(),
#if UNITY_USE_MULTIPLAYER_ROLES
                    MultiplayerRole = instance.m_MultiplayerRole
#endif
                }
            };
        }
