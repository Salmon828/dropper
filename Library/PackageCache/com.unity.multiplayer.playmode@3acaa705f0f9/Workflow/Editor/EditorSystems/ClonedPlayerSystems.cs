...
namespace Unity.Multiplayer.Playmode.Workflow.Editor
{
    ...
    class ClonedPlayerSystems
    {
        ...
        internal ClonedPlayerSystems()
        {
            ...
            const string k_FirstLaunchKey = "MPPM_Clone_FirstLaunch";

            // Only apply mute player setting from SystemDataStore on the launch of this clone, not on every script reload
            if (!SessionState.GetBool(k_FirstLaunchKey, false))
            {
                EditorUtility.audioMasterMute = SystemDataStore.GetClone().GetMutePlayers();
                SessionState.SetBool(k_FirstLaunchKey, true);
            }
